...
const profile_info_from_uuid = (uuid, option) => {
      ...
      .then(body => {
        ...
        reqs.GET("https://api.mojang.com/user/profiles/" + uuid + "/names", {json: true})
          .then(body => {
            ...
            if(option === "RAW_RESULTS") {
              resolve(main_user_data)
            }
            else {
...
module.exports = class MCAPI_PLAYERS {
   ...
   */
  static get_from_uuid(uuid, option) {
    return new Promise((resolve, reject) => {
      profile_info_from_uuid(uuid, option)
        .then(resolve)
        .catch(reject)
